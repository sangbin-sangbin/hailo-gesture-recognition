...
class HandTracker:
    def __init__(
        self,
        pd_xml="/home/intel/Works/gesture-recognition/openvino_utils/mediapipe_models/palm_detection_FP16.xml",
        pd_device="CPU",
        pd_score_thresh=0.6,
        pd_nms_thresh=0.3,
        lm_xml="/home/intel/Works/gesture-recognition/openvino_utils/mediapipe_models/hand_landmark_FP16.xml",
        lm_device="CPU",
    ...
    def inference(self, frame):
        ...
        frame_nn = cv2.resize(
            frame.astype(np.float32), (self.pd_w, self.pd_h), interpolation=cv2.INTER_AREA
        )

        # Transpose hxwx3 -> 1x3xhxw
        frame_nn = np.transpose(frame_nn, (2, 0, 1))[None,]

        self.dataset.append(frame_nn[0].tolist())
        ...
        # print("hand tracker")
        # print(inference[self.pd_scores].shape)
        # print(inference[self.pd_scores])
    ...
    def save(self):
        import json
        print('data len:', len(self.dataset))
        with open("./data.json", "w") as json_file:
            json.dump({"data": self.dataset}, json_file)